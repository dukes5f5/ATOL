...
from create_icao_xml import icao_xml
...
#xml Runway Generator for Google Earth Demonstration
def makerunway_xml(df2proc, icao, TD_dist_ft, STP_dist_ft, G_Slope, TP_Alt, RWY_W, GA_Spd, Dep_dist_nm):  
    ...
    for _, row in df2proc.iterrows():
        ...
        setup_kwargs['TCH'] = round(math.tan(math.radians(float(G_Slope))) * float(TD_dist_ft), 0)
        ...
        if pd.isna(row['hi_disp_thld']): #if no displaced threshold
            ...
            #TT80 POINTS
            stpt_a = hi_elev_ft
            intermediate_points, tt80df, total_distance, rwy_slope_rad = generate_equidistant_points(stpt_lat, stpt_long, stpt_a, 
                                                                            lo_wgs_lat, lo_wgs_long, row['lo_elev'], 
                                                                            num_points=15)
            ...
            for tt80index, tt80row in tt80df.iterrows():
                tt80df.at[tt80index, 'ptEGM96alt'] = tt80row['elevation_m'] + tt80row['emg96ft']
            ...
            intermediate_points, tt80df, total_distance, rwy_slope_rad = generate_equidistant_points(stpt_lat, stpt_long, stpt_a, 
                                                                            lo_wgs_lat, lo_wgs_long, row['lo_elev'], num_points=15)
        ...
        xml_output_RWY += icao_xml(UUID, runways=all_kargs, icao=icao, finalize=False)
        
    xml_output_PRE = icao_xml(UUID, runways=None, icao=icao, setup=True)
    xml_output_POST += icao_xml(UUID, runways=None, icao=icao, finalize=True)
    
    xml_path = f"C:/Users/dukes/Sync/AriesCI/ATOLv4_2025/FINAL/RETURNS/{icao}/{icao}_runways.xml"
    
    with open(xml_path, "w", encoding="utf-8") as xml_file:
        xml_file.write(xml_output_PRE + xml_output_RWY + xml_output_POST)
